...
def test_all_libs():
  ...
  for lib in libs:
    ...
    assert(len(vt) == 3)
    assert(_all_float(vt))         # v is ints, but output is float

    vt_s = _trans(v, ts, lib)
    assert(isinstance(vt_s, list))  # Outer type of v is preserved
    assert(len(vt_s) == 3)
    assert(_all_float(vt))           # v is ints, but vt is float
    assert(vt == vt_s)              # Outputs match when time formats differ


    vt = _trans(np.array(v), np.array(t), lib)
    ...
    vt_s = _trans(np.array(v), np.array(ts), lib)
    ...
    vt = _trans(v, np.array(t), lib)
    assert(isinstance(vt, list))       # Outer type of input is preserved
    assert(len(vt) == 3)
    assert(_all_float(vt))              # v is ints, but vt is float

    vt_s = _trans(v, np.array(ts), lib)
    assert(isinstance(vt, list))        # Outer type of input is preserved
    assert(len(vt_s) == 3)
    assert(len(vt_s) == 3)
    assert(_all_float(vt_s))             # v is ints, but vt is float
    assert(vt == vt_s)                  # Outputs match when time formats differ
    ...
    vt = _trans(np.array(v), [t, t], lib)
    assert(isinstance(vt, np.ndarray)) # Outer type of input is preserved
    assert(vt.shape == (2, 3))         # Number of rows matches number of timestamps

    vt = _trans(v, np.array([t, t]), lib)
    assert(isinstance(vt, list))    # Outer type of input is preserved
    assert(len(vt) == 2)            # Number of output items matches number of timestamps
    assert(isinstance(vt[0], list)) # Inner type of input is preserved
    assert(isinstance(vt[1], list)) # Inner type of input is preserved

    vt = _trans(np.array(v), np.array([t, t]), lib)
    ...
    assert(len(vt[0]) == 3)
    assert(len(vt[1]) == 3)
    assert(_all_float(vt))           # Input is ints, but output is float

    vt = _trans([v, v], np.array([t, t]), lib)
    assert(isinstance(vt, list))    # Outer type of input is preserved
    assert(isinstance(vt[0], list)) # Inner type of input is preserved
    assert(isinstance(vt[1], list)) # Inner type of input is preserved
    assert(len(vt) == 2)
    ...
    vt = _trans(np.array([v, v]), [t, t], lib)
    assert(isinstance(vt, np.ndarray))
    assert(vt.shape == (2, 3))  # Number of rows matches number of input rows

    vt = _trans(np.array([v, v]), np.array([t, t]), lib)
    assert(isinstance(vt, np.ndarray))    # Outer type of input is preserved
    assert(vt.shape == (2, 3))            # Number of rows matches number of timestamps
    assert(vt[0].dtype == np.double)
    assert(vt[1].dtype == np.double)

    vt1 = _trans(v, [t, t], lib)
    vt2 = _trans(v, [ts, ts], lib)
    assert(vt1 == vt2)

    vt1 = _trans(v, np.array([ts, ts]), lib)
    vt2 = _trans(v, [t, t], lib)
    assert(vt1 == vt2)

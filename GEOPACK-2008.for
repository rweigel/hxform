...
C
      ...
      ELSE IF (EXNAME == 'cons_mag') THEN
        CALL CONS_MAG(PARMOD, BXGSW, BYGSW, BZGSW)
      END IF
...
!f2py intent(hide) :: L,  XF, YF, ZF
      CHARACTER(30) :: EXNAME, INNAME
      REAL*4 :: X, Y, Z, DIR, DSMAX, ERR, RLIM, R0,XF,YF,ZF,
...
C
      SUBROUTINE SPHtoCAR_08_V (R,THETA,PHI,X,Y,Z,N)
        ...
        REAL*4 R(N), THETA(N), PHI(N), X(N), Y(N), Z(N)
        INTEGER N
...
!f2py intent(hide) :: N
        ...
        SUBROUTINE CARtoSPH_08_V (R,THETA,RHO,X,Y,Z,N)
          ...
          REAL*4 R(N), THETA(N), RHO(N), X(N), Y(N), Z(N)
          INTEGER N

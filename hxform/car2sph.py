def car2sph(*args):
  ...
  Usage:
    ...
    rtp = car2sph(xyz) where xyz is a 2D array-like object with shape (N, 3).
    xyz may be a list/tuple of lists/tuples or 1D numpy arrays.
  ...
  colat = 90.0 - (180.0/np.pi)*np.arccos(z/r)
  lon = (180.0/np.pi)*np.arctan2(y, x)

  return matrix2components(*args, np.column_stack([r, colat, lon]))

...
double calcG01(double fracYearIndex, double fracYear)
{
	static int g01[NUM_IGRF_YEARS_DEFINED] =
		{-31543, -31464, -31354, -31212, -31060, -30926, -30805, -30715,
		 ...
		 -29992, -29873, -29775, -29692, -29619, -29555, -29497, -29441, -29405};

  printf("%f\n", fracYear);
  printf("IGRF interval = %d; g01 = %d\n", (int)floor(fracYearIndex), g01[(int)floor(fracYearIndex)]);
  printf("IGRF interval = %d; g01 = %d\n", (int)ceil(fracYearIndex), g01[(int)ceil(fracYearIndex)]);
...
double calcH11(double fracYearIndex, double fracYear)
{
	static int h11[NUM_IGRF_YEARS_DEFINED] = 
		{5922, 5909, 5898, 5875, 5845, 5817, 5808, 5812, 5821, 5810, 5815,
		 5820, 5791, 5776, 5737, 5675, 5604, 5500, 5406, 5306, 5186, 5078,
                 4944, 4797, 4653};
...
{
  // jd = 2415020.5 is 1900-01-01T00:00:00 based on https://pdc.ro.nu/mjd.html#jd
  // jd = 2451544.5 is 2000-01-01T00:00:00 based on https://pdc.ro.nu/mjd.html#jd
  printf("%d %d %d %d %d %d\n", yyyy, mm, dd, hh, mm2, ss);
  printf("jd = %f\n", gregorian_calendar_to_jd(yyyy, mm, dd, hh, mm2, ss));
  printf("jd relative to 2000-01-01T00:00:00 = %f\n", (gregorian_calendar_to_jd(yyyy, mm, dd, hh, mm2, ss) - 2451544.5));
  printf("jd seconds relative to 2000-01-01T12:00:00Z = %ld\n", cxRound((gregorian_calendar_to_jd(yyyy, mm, dd, hh, mm2, ss) - 2451544.5) * 86400));
	return cxRound((gregorian_calendar_to_jd(yyyy, mm, dd, hh, mm2, ss) - 2451544.5) * 86400);
}
